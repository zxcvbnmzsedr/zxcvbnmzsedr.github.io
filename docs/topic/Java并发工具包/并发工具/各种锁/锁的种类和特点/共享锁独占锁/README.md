...
![](/localized-assets/2021-11-29-13-04-32-image-1b0b062e40e1.png)
...
![读锁插队成功](assets/读锁插队成功.png)

### 第二种策略：不允许插队

这种策略认为由于线程 3 已经提前等待了，所以虽然线程 5 如果直接插队成功，可以提高效率，但是我们依然让线程 5 去排队等待：

![读锁不允许插队](assets/读锁不允许插队.png)按照这种策略线程 5 会被放入等待队列中，并且排在线程 3 的后面，让线程 3 优先于线程 5 执行，这样可以避免“饥饿”状态。

这对于程序的健壮性是很有好处的，直到线程 3 运行完毕，线程 5 才有机会运行，这样谁都不会等待太久的时间。

![读锁不允许插队-结果2](assets/读锁不允许插队-结果2.png)
